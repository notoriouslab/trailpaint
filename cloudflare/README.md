...
**作用**：
1. 攔截帶 `Accept: text/markdown` header 的請求，改送對應的 `.md` 檔案（而非 HTML）
2. 修 `.well-known/api-catalog` Content-Type → `application/linkset+json`（RFC 9727）
3. 修 `.well-known/trailpaint` Content-Type → `application/json`（GitHub Pages 對無副檔名檔預設 octet-stream）
4. 在可發現頁面注入 RFC 8288 `Link:` header，列 llms.txt / agent-card / agent-skills / api-catalog / **trailpaint discovery** / **project schema** / sitemap

其他請求全部 pass-through 到 GitHub Pages。
